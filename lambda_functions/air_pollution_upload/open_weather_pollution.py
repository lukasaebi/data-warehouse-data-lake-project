...
coordinates = {
    city["city_name"]: {"lat": city["lat"], "lon": city["lon"]} for city in cities_data
}
...
def fetch_air_pollution(coordinates, api_key, time_ranges):
    """
    Retrieves air pollution data for the specified time ranges.
    Filters only data from 6:00 to 9:00 UTC.
    """
    all_data = []
        ...
        for start, end in time_ranges:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"Fehlerhafte Anfrage für {place} ({lat}, {lon}). Statuscode: {response.status_code}"
                    )
