...
# Local directory for storing arrivals data
ARRIVALS_DIR = os.path.join("aviations3", "arrivals")
...
# Fetch arrivals data for a given IATA code and date range
def fetch_arrivals_data(iata_code, start_date, end_date):
    ...
    try:
        ...
        print(f"Error fetching arrivals data for {iata_code}: {e}")
        return None
...
# Main function for processing arrivalss
def process_arrivalss():
    ...
    for city_info in CITIES:
        ...
        for iata_code in iata_codes:
            for start_date, end_date in date_segments:
                data = fetch_arrivals_data(iata_code, start_date, end_date)
                if data:
                    date_range = f"{start_date}_to_{end_date}"
                    save_data_locally(data, ARRIVALS_DIR, city, iata_code, date_range)
                else:
                    print(f"No arrivals data for {iata_code} in {city} from {start_date} to {end_date}")

# Entry point for processing arrivalss
def main():
    process_arrivalss()
